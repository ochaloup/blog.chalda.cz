...
For an usual contract it's a good practice for contract defining a metadata providing
about the program. Besides others where is the source code and how to contact the author.
This should be easily to parse and it's useful for security researchers.
...
The Rust program adds `solana-security-txt = "1.1.0"` dependency to the `Cargo.toml` file
and then adds the metadata into the library source code https://github.com/marinade-finance/solana-program-library/commit/93d808aac6a69c253edc3cddc6e15f5a2c81be24[with macro definition `security_txt!`]. A simple example cound b e

[source,rust]
----
/// solana-security-txt definition
...
The good part is that this security txt standard is integrated within
