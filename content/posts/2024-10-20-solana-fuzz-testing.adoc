...
The honggfuzz generate random bytes
...
NOTE: when `Anchor.toml` is at different place from where the Trident tests are stored
      then one can use option `trident fuzz --root <anchor-project-root>`.

By default the fuzzing does not care about the results and it just generates
data until is stopped manually by `CTRL^C` or by specific condition in the configuration.
That's where the
link:https://github.com/Ackee-Blockchain/trident/blob/0.7.0/examples/fuzz-tests/hello_world/Trident.toml[`Trident.toml`^]
comes to play.

The fuzz tests may get stopped either on number of
...
Otherwise the test is fuzzing until other condition is held
and the crashes can be evaluated all together.

At the `[fuzz]` section is good to enable `fuzzing_with_stats`
that provides a summary of the instructions that were run.

=== Trident.toml configuration for debugging

A good thing for debugging purposes is to setup the
lintk:https://www.youtube.com/watch?v=gMk6hm0x44M&t=1276s[following parameters^]:

[source,toml]
----
[honggfuzz]
# lower number of interations to be comprehensible
iterations = 10  
...
This setup makes the fuzzing to print all the Solana program log
information to console.

== Evaluation crashes

For evaluation of errors one need to check the crashfiles that are generated into
`trident-tests/fuzz_tests/fuzzing/hfuzz_workspace/fuzz_0/*.fuzz`.
For that it's needed to run cli command
...
With that one is booted into `lldb` console (write `q<ENTER>` to quit).
After execution we can usually end up in the end of printing where
a long lldb stacktrace can be seen.
We need to scroll up to the part where information about Solana program
processing is shown. The we can see title
...
That informs what parameters where used on instruction invocation
and usually the program log can shed light on reasons of the failure.
...
* On `run-debug` for a crash file is printed `This crashfile didn't trigger any panics...` and
  ...
  of the panic.
  This seems to be a case when Solana program exists with an `Error` and not with `panic!`.
  It seems to be all ok otherwise and one should check the Solana program logs what happened.

* `warning: This version of LLDB has no plugin for the language "rust". Inspection of frame variables will be limited.`.
  Not sure on tackle this. It seems `rust-lldb` could hep but installation is not straight forward.
