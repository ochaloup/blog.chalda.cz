...
If you reference your plugin in project to get it executed you will put it to the project `pom.xml` as
...
NOTE: you can define multiple `<execution>` elements (e.g. with different configuration)
and you can use multiple `<goal>` elements to get them executed
...
NOTE: the `<configuration>` element could be part of the `<execution>` too
...
Here we used the simple `@Parameter`. This annotation has several attributes.
We can define the default value as the project build folder and even use the Maven variable
`@Parameter(defaultValue = "${project.build.directory}")`.

A bit trouble is with multi value parameter where `defaultValue` is not evaluated
from the Maven variables.
...
which in turn is not evaluated from Maven configuration property to its value.
...
injected to with `@Parameter` annotation and then queried for such things like build directory.
Thus one way to solve this issue mentioned above is
...
Consult the documentation to check all the annotations and its parameters
...
Normally (I hope I can says so :) the plugin can see dependencies defined in `compile` scope
of the plugin project itself. Not the project it's referenced in (the project the plugin is executed at).

This could be shown when resolved from `PluginDescriptor` Maven class. It's shown in the following code snippet
...
public class MavenPluginTest extends AbstractMojo {
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    try {
        getLog().info(("compile cp: " + this.project.getCompileClasspathElements());
        getLog().info(("runtime cp: " + this.project.getRuntimeClasspathElements());
...
With these I could create my own class loader and particularly say what is scope of the class loading
(you can check notes over here to http://blog.semsur-it.com/2011/11/java-class-loader-and-maven-plugin.html)
...
=== Note on class loader debugging
...
NOTE: bear in mind the _normal_ java classloader first ask parent if the resource is known and then it tries
to resolve it himself. This is reflected in the printing (ie. you can see the child classloader prints
it can work with what the parent classloader is capable too).
...
System.out.printf("%n--%ncontextClassLoader:%s",
  ClassLoaderUtils.showClassLoaderHierarchy(Thread.currentThread().getContextClassLoader()));
System.out.printf("%n--%ncontextClassLoader:%s",
  ClassLoaderUtils.showClassLoaderHierarchy(MavenPluginTest.class.getClassLoader()));
```

This is a short summary how to start and get it working.

As a first step I would recommend to check the nice short tutorial at
https://maven.apache.org/guides/plugin/guide-java-plugin-development.html

== Archetype scaffolding

If you want started writing a Maven plugin you can use the archetype generator

```bash
mvn archetype:generate -DgroupId=org.apache.maven -DartifactId=maven-plugin-test -DarchetypeArtifactId=maven-archetype-mojo -DinteractiveMode=false
```

Maven generates the scaffolder of the Maven plugin project. You can see it makes
dependency on the `org.apache.maven:maven-plugin-api` in `pom.xml`. The Maven plugin
class `or.apache.maven.MyMojo` then uses javadoc annotations to define properties
of the plugin.
The documentation for this approach could be found at
https://books.sonatype.com/mvnref-book/reference/writing-plugins-sect-mojo-params.html.
I haven't written the plugin this way but I changed for using Maven annnotations.
...
<!-- Maven project to work with like `MavenProject`, `PluginDescriptor` etc. -->
<dependency>
    <groupId>org.apache.maven</groupId>
    <artifactId>maven-core</artifactId>
    <version>3.5.2</version>
...
NOTE: I'm not sure about the correct `scope` but provided works for me thus I've
  left it in my plugin for now.

=== Source code definition

The Maven plugin consists from one (or several) classes which inherit from class `org.apache.maven.plugin.AbstractMojo`.
This defines to implement method `execute()` which is called when the plugin is invoked.
The plugin is introduced by annotation `@Mojo` (_Maven plain Old Java Object_ )
marking the class as the Maven plugin, assigning the `goal` making it executable.
You can create multiple Mojo classes in your project and cover more goals.
