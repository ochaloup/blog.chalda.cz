...
* it happens to me time to time that I'm not able to save or publish an article. When looking to `javascript` console it's some 'unanonymous entity was not saved' or similar error. My workaround is to copy text to some text editor on my computer, remove the post completely and re-add it again. I think there could be some trouble when adding content from my local machine but as I didn't dig into the javascript at all.
