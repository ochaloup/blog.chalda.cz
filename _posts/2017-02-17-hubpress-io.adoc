...
but it seems that it's not so true as new versions are coming.
...
. After the first login you can see empty screen... +
  oh, with version `0.8.0` the UI changed so there will be probably info that you
  have 0 posts. But don't be surprised and just create a new article.
...
// TODO: How to add a new theme

And that's mostly it. I'm now learning to work with it.
